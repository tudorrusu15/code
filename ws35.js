document.addEventListener('DOMContentLoaded', function() {
  ...
  function updateGiftUI(imageId, nameId, variant) {
    console.log('updateGiftUI function has been called');
    const name = variant.product.title;
    const imageUrl = variant.image.src;
  ...
  function calculateTotalComparePrice(mainProductCompareAtPrice, quantity) {
    console.log("mainProductCompareAtPrice:", mainProductCompareAtPrice);
...
if (quantity >= 4) {
    ...
    atcButton.addEventListener('click', function() {
        ...
        updateCartDisplay(checkout, compareAtPrice, mainProductCompareAtPrice, quantity);  
        updateGiftDisplay();
        fetchCompareAtPrice(checkout);
        updateCart();
    });
    ...
    if (!checkout) return Promise.reject('Checkout object is undefined'); // Exit if the checkout object is undefined

    const globalId = btoa(`gid://shopify/ProductVariant/${mainProductVariantId}`);
    return fetch("https://4bdc87-2.myshopify.com/api/2023-07/graphql.json", {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            "X-Shopify-Storefront-Access-Token": shopifyAccessToken
    ...
    .then(data => {
        ...
        calculateTotalComparePrice(compareAtPrice, quantity); // Call calculateTotalComparePrice here
    });
}
  ...
  function updateGiftDisplay() {
    ...
    function updateCartDisplay(checkout, compareAtPrice, mainProductCompareAtPrice, quantity) {
        console.log('updateCartDisplay function has been called');
        ...
        const totalComparePrice = calculateTotalComparePrice(mainProductCompareAtPrice, quantity);
        ...
        totalComparePrice.textContent = totalComparePrice.toFixed(2);
...
if (checkout) {
    ...
    .then(compareAtPrice => {
        updateCartDisplay(checkout, compareAtPrice, mainProductCompareAtPrice, quantity);
    }); // Fetch the compare-at price using the stored checkout object
}
});
