...
const APPLY_DISCOUNT_CODE_MUTATION = `mutation applyDiscountCodeToCheckout($checkoutId: ID!, $discountCode: String!) {
...
const checkoutButton = document.getElementById('checkout-button'); // Assuming this is the checkout button
...
function updateCart() {
  ...
  .then(data => {
    ...
    [freeShippingCode, additionalDiscountCode].filter(Boolean).forEach(discountCode => {
      fetch("https://4bdc87-2.myshopify.com/api/2023-07/graphql.json", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
