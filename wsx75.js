...
function updateCart() {
  ...
  ongoingApiCalls++; // Increment the counter
  ...
  .then(data => {
    console.log("Full checkoutCreate response:", data.data.checkoutCreate);
    const checkoutId = data.data.checkoutCreate.checkout.id;

    // Retrieve the discount codes
    const percentageCode = document.getElementById('coupon-code-percentage').value;
    const dollarCode = document.getElementById('coupon-code-dollar').value;
    const freeShippingCode = document.getElementById('free-shipping-code').value;

    let chosenDiscountCode;
    if (percentageCode && percentageCode.trim() !== "") {
      chosenDiscountCode = percentageCode;
    } else if (dollarCode && dollarCode.trim() !== "") {
      chosenDiscountCode = dollarCode;
    }

    const discountCodes = [chosenDiscountCode, freeShippingCode].filter(Boolean);

    console.log("Checkout ID:", checkoutId);

    // Apply the discount codes
    const applyDiscountCodeSequentially = (index) => {
      if (index >= discountCodes.length) {
        ...
        ongoingApiCalls--; // Decrement the counter
      ...
      applyDiscountCode(checkoutId, discountCodes[index])
      .then(() => applyDiscountCodeSequentially(index + 1));
    };

    applyDiscountCodeSequentially(0);
  })
  .catch(error => {
...
function applyDiscountCode(checkoutId, discountCode) {
  ...
  .then(response => response.json())
  .then(data => {
    console.log(`Response for discount code ${discountCode}:`, data);
        if (data.errors && data.errors.length > 0) {
            console.error(`Error applying discount code ${discountCode}:`, data.errors);
        }
