document.addEventListener('DOMContentLoaded', function() {
  ...
  function fetchGift2Details() {
    console.log("fetchGift2Details function called");
...
if (storedCheckout) {
    ...
    const mainProductQuantity = checkout.lineItems.edges[0]?.node.quantity || 0;

    // Find the HTML element where you want to display the quantity
    const quantityElement = document.getElementById('item-quantity');

    // Update the content of the quantity element
    quantityElement.textContent = mainProductQuantity;
}
...
if (quantity >= 4) {
    ...
    atcButton.addEventListener('click', function() {
        ...
        const existingItem = .find(item => item.variantId === mainProductVariantId);
        if (existingItem) {
