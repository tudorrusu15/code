...
function updateCart() {
  ...
  function updateGiftDisplay() {
    ...
    // Update the item image, name, and quantity
    ...
    function animatePriceChange(element, currentPrice, newPrice) {
        ...
        function animate(time) {
            ...
            const progress = Math.sin((timeFraction * Math.PI) / 2); // easeOutSin easing function
            const animatedValue = currentPrice + progress * (newPrice - currentPrice);

            element.textContent = `${currencyCode} ${animatedValue.toFixed(2)}`;

            if (timeFraction < 1) {
                requestAnimationFrame(animate);
            } else {
                element.textContent = `${currencyCode} ${newPrice.toFixed(2)}`;
            }
        }
    ...
    animatePriceChange(itemPriceElement, parseFloat(itemPriceElement.textContent.replace(currencyCode, '').trim()), itemPrice * quantity);
    animatePriceChange(itemComparePriceElement, parseFloat(itemComparePriceElement.textContent.replace(currencyCode, '').trim()), compareAtPrice * quantity);
    animatePriceChange(finalPriceElement, parseFloat(finalPriceElement.textContent.replace(currencyCode, '').trim()), finalPrice);
    animatePriceChange(totalComparePriceElement, parseFloat(totalComparePriceElement.textContent.replace(currencyCode, '').trim()), totalComparePriceValue);
    animatePriceChange(itemSavings, parseFloat(itemSavings.textContent.replace(currencyCode, '').trim()), totalSavings);
}
