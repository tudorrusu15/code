...
if (checkoutURL) {
    ...
    const additionalDiscountCodeElement = document.getElementById('discount-code');
    ...
    if (data.data.checkoutCreate.checkoutUserErrors.length > 0) {
        return;
          ...
          // Fetch the compare-at price and then update the cart display
            ...
            // If the quantity is 1 or more, modify the checkout URL to include the discount code
        if (quantity >= 1) {
            checkoutURL += `&discount=${freeShippingCode}`;
        }

        if (additionalDiscountCode) {
            checkoutURL += `&additional_discount=${additionalDiscountCode}`;
        }
