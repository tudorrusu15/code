...
function updateCart() {
  ...
  .then(data => {
    ...
    const applyDiscountCode = (index) => {
      if (index >= discountCodes.length) {
        ...
        toggleCart();
        fetchCompareAtPrice(checkout, finalPrice);    
    return;
  }
      ...
      .then(data => {
        if (data.data && data.data.checkoutDiscountCodeApplyV2 && data.data.checkoutDiscountCodeApplyV2.checkout) {
          ...
          // Update the final price with discounts applied
          finalPrice = parseFloat(data.data.checkoutDiscountCodeApplyV2.checkout.totalPriceV2.amount);
  ...
  fetchCompareAtPrice(checkout, finalPrice);
    ...
    function fetchCompareAtPrice(checkout, imageId, nameId, variant, finalPrice) {
    ...
    .then(data => {
        ...
        updateCartDisplay(checkout, compareAtPrice, finalPrice)
        fetchGift2Details();
        fetchGift3Details();
        fetchGift4Details();
    });
  ...
  function updateGiftDisplay() {
    ...
    function updateCartDisplay(checkout, compareAtPrice, finalPrice) {
    const quantityElement = document.getElementById('item-quantity');
    const quantity = parseInt(quantityElement.textContent, 10);
    ...
    if (checkout) {
        ...
        fetchCompareAtPrice(checkout, finalPrice); // Fetch the compare-at price using the stored checkout object
    }
