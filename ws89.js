...
function updateCart() {
  ...
  .then(data => {
    ...
    function applyDiscountCode(index, checkout, compareAtPrice) {
  if (index >= discountCodes.length) {
    ...
    toggleCart();
    updateCartDisplay(checkout, compareAtPrice); // Update the cart display with the final checkout object and compareAtPrice
    return;
  }
    ...
    // Fetch the compareAtPrice
  fetchCompareAtPrice(checkout).then(compareAtPrice => {
    // Start applying discount codes, passing the compareAtPrice
    applyDiscountCode(0, checkout, compareAtPrice);
  });
