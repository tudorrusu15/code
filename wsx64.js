...
function updateCart() {
  ...
  .then(data => {
    ...
    // Apply the chosen discount code
    let discountCode;
    ...
    if (discountCode) {
      return applyDiscountCode(checkoutId, discountCode);
    } else {
      return Promise.resolve(data); // Continue without applying this discount
    }
  })
  .then(data => {
    // Apply the free shipping code if it exists
    const freeShippingCode = document.getElementById('free-shipping-code').value;
    if (freeShippingCode && freeShippingCode.trim() !== "") {
      return applyDiscountCode(data.data.checkoutCreate.checkout.id, freeShippingCode);
    } else {
      return Promise.resolve(data); // Continue without applying this discount
    }
  })
