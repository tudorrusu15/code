...
if (variant) {
    ...
    $({ countNum: parseFloat(itemPriceElement.textContent.replace('$', '')) }).animate({
        countNum: itemPrice * quantity
    },
    {
        duration: 500,
        easing: 'linear',
        step: function (now) {
            itemPriceElement.textContent = '$' + Math.round(now);
        }
    });

    // Animate the item compare price
    $({ countNum: parseFloat(itemComparePriceElement.textContent.replace('$', '')) }).animate({
        countNum: compareAtPrice * quantity
    },
    {
        duration: 500,
        easing: 'linear',
        step: function (now) {
            itemComparePriceElement.textContent = '$' + Math.round(now);
        }
    });

    // Animate the final price
    $({ countNum: parseFloat(finalPriceElement.textContent.replace('$', '')) }).animate({
        countNum: finalPrice
    },
    {
        duration: 500,
        easing: 'linear',
        step: function (now) {
            finalPriceElement.textContent = '$' + Math.round(now);
        }
    });

    // Animate the total compare price (assuming you have a regular-price element)
    $({ countNum: parseFloat(totalComparePriceElement.textContent.replace('$', '')) }).animate({
        countNum: compareAtPrice * quantity
    },
    {
        duration: 500,
        easing: 'linear',
        step: function (now) {
            totalComparePriceElement.textContent = '$' + Math.round(now);
        }
    });
