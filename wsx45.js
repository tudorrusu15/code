...
if (checkoutURL) {
    ...
    const percentageCode = document.getElementById('coupon-code-percentage');
    const dollarCode = document.getElementById('coupon-code-empty').value;
    ...
    const compareAtPrice = document.getElementById('main-product-compare-price').value;
    ...
    const comparePice = mainProductComparePrice + (quantity >= 2 ? gift2PriceValue : 0) + (quantity >= 3 ? gift3PriceValue : 0) + (quantity >= 4 ? gift4PriceValue : 0);
    ...
    itemSavings.textContent = `$${totalSavings.toFixed(2)}`;
}
...
function updateCart() {
  ...
  .then(data => {
    ...
    const discountCode = percentageCode || dollarCode;
