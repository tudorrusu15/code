  ...
  function updateGiftDisplay() {
    ...
    function animatePriceChange(element, currentPrice, newPrice) {
        ...
        },
        {
            ...
            easing: 'linear',
            step: function (now) {
                element.textContent = `${currencyCode} ${now.toFixed(2)}`;
            }
        });
    }

    // Animate price changes
    animatePriceChange(itemPriceElement, parseFloat(itemPriceElement.textContent.replace(currencyCode, '')), itemPrice * quantity);
    animatePriceChange(itemComparePriceElement, parseFloat(itemComparePriceElement.textContent.replace(currencyCode, '')), compareAtPrice * quantity);
    animatePriceChange(finalPriceElement, parseFloat(finalPriceElement.textContent.replace(currencyCode, '')), finalPrice);
    animatePriceChange(totalComparePriceElement, parseFloat(totalComparePriceElement.textContent.replace(currencyCode, '')), totalComparePriceValue);
    animatePriceChange(itemSavings, parseFloat(itemSavings.textContent.replace(currencyCode, '')), totalSavings);
}
