...
function updateCart() {
  ...
  .then(data => {
    ...
    function applyDiscountCode(index, checkout) {
  ...
  .then(data => {
    if (data.data && data.data.checkoutDiscountCodeApplyV2 && data.data.checkoutDiscountCodeApplyV2.checkout) {
      // Update checkout object with the applied discount
      checkout = data.data.checkoutDiscountCodeApplyV2.checkout;
      // Update the UI with the new checkout object
      updateCartDisplay(checkout, compareAtPrice);
    }

    // Apply the next discount code
    applyDiscountCode(index + 1, checkout);
  });
}
