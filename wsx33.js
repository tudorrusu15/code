...
if (checkoutURL) {
    ...
    const GIFT_DETAILS_QUERY = `
{
  gift2: node(id: "${btoa(`gid://shopify/ProductVariant/${gift2VariantId}`)}") {
    ... on ProductVariant {
      compareAtPriceV2 {
        amount
        currencyCode
  ...
  gift3: node(id: "${btoa(`gid://shopify/ProductVariant/${gift3VariantId}`)}") {
    ... on ProductVariant {
      compareAtPriceV2 {
        amount
        currencyCode
  ...
  gift4: node(id: "${btoa(`gid://shopify/ProductVariant/${gift4VariantId}`)}") {
    ... on ProductVariant {
      compareAtPriceV2 {
        amount
        currencyCode
...
`;
  ...
  function fetchGiftDetails() {
    fetch("https://4bdc87-2.myshopify.com/api/2023-07/graphql.json", {
        ...
        body: JSON.stringify({
            query: GIFT_DETAILS_QUERY
        })
    })
    .then(response => response.json())
    .then(data => {
        console.log(data);
        processGiftData(data.data.gift2, 'gift-2-compare-price', 'gift-image-2', 'gift-name-2', 2);
        processGiftData(data.data.gift3, 'gift-3-compare-price', 'gift-image-3', 'gift-name-3', 3);
        processGiftData(data.data.gift4, 'gift-4-compare-price', 'gift-image-4', 'gift-name-4', 4);
    });
}

  function processGiftData(variant, comparePriceElementId, imageElementId, nameElementId, giftNumber) {
    ...
    document.getElementById(comparePriceElementId).value = compareAtPriceValue;
    updateGiftUI(imageElementId, nameElementId, variant, giftNumber);
}
