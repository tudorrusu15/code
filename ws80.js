...
function updateCart() {
  ...
  const merchandiseId = btoa(`gid://shopify/ProductVariant/${variantId}`);

  // Create line items based on quantity
  let lines = [{ merchandiseId: merchandiseId, quantity: quantity }];
  if (quantity >= 2) {
    lines.push({ merchandiseId: btoa(`gid://shopify/ProductVariant/${gift2VariantId}`), quantity: 1 }); // Add gift 2
  }
  if (quantity >= 3) {
    lines.push({ merchandiseId: btoa(`gid://shopify/ProductVariant/${gift3VariantId}`), quantity: 1 }); // Add gift 3
  }
  if (quantity >= 4) {
    lines.push({ merchandiseId: btoa(`gid://shopify/ProductVariant/${gift4VariantId}`), quantity: 1 }); // Add gift 4
  }

  const variables = {
    input: {
      lines: lines,
      discountCodes: [freeShippingCode, additionalDiscountCode].filter(Boolean) // Include only non-empty codes
    }
  };
  ...
  .then(data => {
    ...
    localStorage.setItem('cart', JSON.stringify(cart)); // Store the cart object

    const merchandise = cart.lines.edges[0].node.merchandise; // Assuming lineItem is the relevant item
    localStorage.setItem('merchandise', JSON.stringify(merchandise)); // Store the merchandise object

    // Fetch the compare-at price and then update the cart display
    fetchCompareAtPrice(cart);

    // Update the checkout button URL
    const checkoutURL = cart.webUrl;
    checkoutButton.href = checkoutURL;

    // Store the checkout URL in local storage
